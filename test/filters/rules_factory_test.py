...
########## ORGANIZATIONS TEST ##########
def test_will_weight_existance_of_organization_at_1():
    repo = sample_repo(topics=GOOD_TOPICS)
    assert rules_factory.organizations_rule(repo, CONFIG)[WEIGHT] == 1


def test_will_weight_no_organization_at_0():
    repo = sample_repo(topics=OK_TOPICS)
    assert rules_factory.organizations_rule(repo, CONFIG)[WEIGHT] == 0


def test_will_weight_no_topics_for_organizations_as_0():
    repo = sample_repo(topics=BAD_TOPICS)
    assert rules_factory.organizations_rule(repo, CONFIG)[WEIGHT] == 0


def test_will_weight_repo_missing_topics_for_organizations_as_0():
    repo = {}
    assert rules_factory.organizations_rule(repo, CONFIG)[WEIGHT] == 0


def test_will_account_for_existing_weight_with_organizations():
    repo = sample_repo(topics=GOOD_TOPICS)
    repo[WEIGHT] = 0.5
    expected_weight = 0.5
    assert rules_factory.readme_rule(repo, CONFIG)[WEIGHT] == expected_weight
